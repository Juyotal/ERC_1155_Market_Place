...
async function main() {
  ...
  // this will use the first account from the "accounts" array in hardhat.config.ts
  // or the first autogenerated account if using Hardhat Network or localhost
  ...
  const sale = await deploy("NFTSale", [registry.address]);
  contracts.push(sale);
  const auction = await deploy("NFTAuction", [registry.address]);
  contracts.push(auction);

  await registry.contract.setContractStatus(nft.address, true);
  console.log("Registering " + chalk.cyan("NFT") + " contract!");
  await registry.contract.setContractStatus(sale.address, true);
  console.log("Registering " + chalk.cyan("Sale") + " contract!");
  await registry.contract.setContractStatus(auction.address, true);
  console.log("Registering " + chalk.cyan("Auction") + " contract!");
  await registry.contract.setSystemWallet(deployer.address);
  console.log("System address set to: " + chalk.greenBright(deployer.address));
